...
def test_load_allowed_networks(scope_file: Path) -> None:
    ...
    assert [str(network) for network in networks] == ["127.0.0.0/8", "192.168.77.0/24"]

def test_allows_configured_lab_ip(scope_file: Path) -> None:
    decision = evaluate_target("192.168.77.42", load_allowed_networks(scope_file))
    assert decision.allowed is True
    assert "192.168.77.0/24" in decision.reason
...
def test_rejects_network_and_broadcast_addresses(scope_file: Path) -> None:
    networks = load_allowed_networks(scope_file)
    assert evaluate_target("192.168.77.0", networks).allowed is False
    assert evaluate_target("192.168.77.255", networks).allowed is False
...
def test_repository_scope_is_safe_by_default() -> None:
    ...
    assert evaluate_target("192.168.77.10", networks).allowed is True
