...
def test_restructured_checkpoints_are_explicit() -> None:
    ...
    l04 = Path("labs/s00/l04-toolchain/README.md").read_text(encoding="utf-8")
    assert "s00-l01-complete-v2" in l01
    assert "s00-l02-complete-v2" in l02
    assert "s00-l03-complete-v2" in l03
    assert "s00-l04-complete-v2" in l04
...
def test_s00_l04_toolchain_assets_are_present() -> None:
    assert Path("labs/s00/l04-toolchain/README.md").is_file()
    assert Path("scripts/env_check.py").is_file()
    assert Path("progress/s00/l04-toolchain-record.md").is_file()
